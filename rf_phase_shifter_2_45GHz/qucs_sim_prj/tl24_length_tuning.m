...
[data] = loadQucsDataSet("tl24_length_tuning.dat")
...
%attenuation
figure;
title ("Attenuation: TL 2,4 length sweep");
...
leg = legend(leg_str)
set(leg, "title", "TL 2,4 length [λ]")
xlim([0,3.3])
grid
print "plots/att_tl24_len_sweep.png"

%phase shift
figure;
title ("Phase shift: TL 2,4 length sweep");
xlabel ("Bias voltage [V]");
ylabel ("Phase shift [°]");
hold on
for ii = 1:length(tl_length)
...
xlim([0,3.3])
grid
print "plots/pha_tl24_len_sweep.png"

%ripple and max phase shift
figure;
title ("Attenuation ripple and phase shift range: TL 2,4 length sweep");
hold on
...
xlabel ("TL 2,4 impedance [λ]");
ylabel (ax(1), "Amplitude ripple [dB]");
ylabel (ax(2), "Maximum phase shift [°]");

grid
print "plots/ripp_and_max_phase_shift_tl24_len_sweep.png"
